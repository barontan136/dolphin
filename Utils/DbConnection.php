...
class DbConnection
{
    ...
    {
        $this->logger->debug(sprintf('[sql:%s]', $query));
        try {
    ...
    public function query($query = '', $params = null, $fetchmode = PDO::FETCH_ASSOC)
    {
        ...
        if ($statement === 'select' || $statement === 'show') {
            ...
            $this->sQuery->closeCursor(); //释放PDOStatement资源
            return $result;
	...
	{
		$field_names = $this->column('DESC ' . $table);
		...
		if ($mode == 'INSERT') {
			...
			foreach ($field_names AS $value) {
				if (array_key_exists($value, $field_values) == true) {
					$fields[] = $value;
					if ($value == 'create_datetime' || $value == 'update_datetime') {
						$values[] = "now()";
					} else {
     ...
     * 执行sql语句
     * @param string $sql
     * @return mixed 失败false成功返回影响行数
     */
    public function _execute( $sql) {
        $this->logger->debug(sprintf('[sql:%s]', $sql));
        //$this->logsql( $sql);
        $stm = $this->pdo->prepare( $sql);
        $result = $stm->execute();
    ...
    public function _insert($table_name, $data) {
        ...
        foreach( $data as $key=>$value) {
            $aFields[] = sprintf( "`%s`", $key);
            $aValues[] = $this->_quote( $value);
        }
        $sFields = implode( ',', $aFields);
        $sValues = implode( ',', $aValues);

        $sql = "insert into {$table_name}( {$sFields}) values( {$sValues})";
     ...
     */
    public function _update($table_name, array $data, array $where=null) {
        if( empty( $data)) {
            return 0;
        }
        ...
        $sets = array();
        foreach( $data as $key=>$value) {
            $sets[] = sprintf( "`%s`=%s", $key, $this->_quote( $value));
        }
        $sets = implode( ",", $sets);

        $sWhere = $this->_getStrWhere( $where);
        if( $sWhere) {
            $sWhere = ' where '.$sWhere;
        }

        $sql = "update {$table_name} set {$sets} {$sWhere}";
     ...
     */
    public function _delete($table_name, array $where) {
        $sWhere = $this->_getStrWhere( $where);
        if( $sWhere) {
            $sWhere = ' where '.$sWhere;
        }

        $sql = "delete from `{$table_name}` {$sWhere}";
     ...
     */
    public function _getStrWhere( array $where=null) {
        $sWhere = '';
        if (!empty( $where)) {
            ...
            foreach( $where as $key=>$value) {
                ...
                if(is_array( $value)) {
                    foreach( $value as $subkey=>$subvalue) {
                        $value[$subkey] = $this->_quote( $subvalue);
                    }
                    $svalue = implode( ',', $value);
                    $aWhere[] = '(' . str_replace( '?', $svalue, $key) . ')';
                } else {
                    $aWhere[] = '(' . str_replace( '?', $this->_quote($svalue), $key) . ')';
                }
            }
